...
public class VistaPrincpal extends JFrame {
	...
	public VistaPrincpal() {
		...
		btnExportarArchivoTokens.addActionListener(new ActionListener() {
			...
			/*//yulia	
			 * String pErrores=textPaneErrores.getText();
				String m="No hay errores";
				if (pErrores==m) {
					JOptionPane.showMessageDialog(null, "Entra a no hay errores");
				}else {
				...
				if(!tokens.isEmpty()) {
					try {
						File fichero = new File("C:\\Users\\ADMIN\\Documents\\automatas\\Proyecto analizador\\Analizadores Final\\src\\tokens.txt"); // RUTA  LAP 
...
//				tokensSeparados.add("tk_BEGIN");
				...
				for (int j = 0; j < pila.size(); j++) {
					...
					if(!nodos.isEmpty()) {
						try {
							File fichero = new File("C:\\Users\\ADMIN\\Documents\\automatas\\Proyecto analizador\\Analizadores Final\\src\\nodosSintactico.txt"); // RUTA  LAP 
							FileOutputStream fos = new FileOutputStream(fichero);
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(baderaErrorSemantico==false) {
					if(!nodos.isEmpty()) {
						try {
							File fichero = new File("C:\\Users\\ADMIN\\Documents\\automatas\\Proyecto analizador\\Analizadores Final\\src\\arbolSemantico.txt"); // RUTA  LAP 
							FileOutputStream fos = new FileOutputStream(fichero);
...
//Metodos 	
	...
	public  void AnalisisSitactico(int i) {
		...
		}else if(tokensSeparados.get(i).equals("tk_ESCRIBIR") || 
				tokensSeparados.get(i).equals("tk_LEER")) {
		...
		}else if(tokensSeparados.get(i).equals("tk_ENTERO") || 
