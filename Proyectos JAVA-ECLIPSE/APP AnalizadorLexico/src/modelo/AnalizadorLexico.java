...
public class AnalizadorLexico extends JFrame {
	...
	public AnalizadorLexico(){
		...
		txp = new JTextArea();
		txp.setFont(new Font("Arial", Font.PLAIN, 14));
		jsp = new JScrollPane();
		jsp.setFont(new Font("Arial", Font.PLAIN, 14));
		...
		txp3 = new JTextArea();
		txp3.setFont(new Font("Arial", Font.PLAIN, 14));
		jsp3 = new JScrollPane();
		jsp3.setFont(new Font("Arial", Font.PLAIN, 14));
		jsp3.setBounds(182, 567, 881, 104);
		jsp3.setViewportView(txp3);
		getContentPane().add(jsp3);

		txp2 = new JTextArea();
		txp2.setFont(new Font("Arial", Font.PLAIN, 14));
		jsp2 = new JScrollPane();
		jsp2.setFont(new Font("Arial", Font.PLAIN, 14));
		...
		btnGenerarTokens.addActionListener(new ActionListener() {
			...
			if (txp.getText().isEmpty()) {
				System.out.println("NO generar tokens");
				JOptionPane.showMessageDialog(null, "NO se pueden generar tokens");
			} else {
				System.out.println("generar tokens");
				generarArchivo();
				cToken = 0;
				cLineas = 1;
				String errores = "";
		...
		for (int i = q; i < c.length; i++) {
			if (comparar(c[i], numeros)) {
				if (decimal == 0) {
					...
					errores += "\nERROR LEXICO (" + cLineas + "): Numero invalido";
					i = recorrer(i);
					termino = true;
				}
			} else if (c[i] == '.') {
					...
					if (ent <= 5) {
						if (decimal == 1) {
							if (dec <= 2) {
								...
								errores += "\nERROR LEXICO (" + cLineas + "): Numero invalido, parte decimal mayor a 2 digitos";
							}
						} else {
							tokens[cToken] = cadena;
							cToken++;
						...
						errores += "\nERROR LEXICO (" + cLineas + "): Numero invalido, parte entera mayor a 5 digitos";
					}
				} else {
					errores += "\nERROR LEXICO (" + cLineas + "): Numero invalido";
				}
	...
	public void generarArchivo() {
			...
			for (int j = 0; j < palabrasRes.length; j++) {
				if (tokens[i].equals(palabrasRes[j])) {
					...
					if (tokens[i].indexOf('.') >= 0) {
						archivo += "(tk_NumeroReal, " + tokens[i] + ")\n";
						j = palabrasRes.length;
					} else {
						if (comparar2(tokens[i], letrasMinu) || comparar2(tokens[i], letrasMayu)) {
							archivo += "(tk_IDENTIFICADOR, " + tokens[i] + ")\n";
							j = palabrasRes.length;
						} else {
							archivo += "(tk_NumeroEntero, " + tokens[i] + ")\n";
							j = palabrasRes.length;
						}
					}
				}
	  ...
	  private void crearTablaSimbolo(String texto) {
	        try {
	            FileWriter escrito = new FileWriter("C:\\Users\\mitsh\\Documents\\Eclipse\\APP AnalizadorLexico\\sample.txt");
	...
	public void mostrar() {
		...
		crearTablaSimbolo(tabla);
	}
	public void escribeTablaSimbolos() throws IOException {
		String filePath = "C:\\Users\\mitsh\\Documents\\Eclipse\\APP AnalizadorLexico\\sample.txt";
	      String input = null ;
	      Scanner sc = new Scanner(new File(filePath));
	      FileWriter writer = new FileWriter("C:\\Users\\mitsh\\Documents\\Eclipse\\APP AnalizadorLexico\\output.txt");
	      Set set = new HashSet();
	...
	public void muestraTablaSimbolos() {
		String codigo = new String(), path ="C:\\Users\\mitsh\\Documents\\Eclipse\\APP AnalizadorLexico\\output.txt";
